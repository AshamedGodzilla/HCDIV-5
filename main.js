...
function createChart(selector, data, config) {
    ...
    svg.selectAll('.dot-rate')
        ...
        .attr('cy', d => y1(d.rate))
        .attr('r', 4)
        .style('fill', config.colors?.rate || '#ff6b6b')
        .on('mouseover', function(event, d) {
            ...
            tooltip.html(
                `Year: ${d.year}<br/>` +
                `${config.title}: ${config.tooltipFormat ? config.tooltipFormat(d.rate) : d.rate.toFixed(2)}`
            )
    ...
    svg.selectAll('.dot-gdp')
        ...
        .attr('cy', d => y2(d.gdp))
        .attr('r', 4)
        .style('fill', config.colors?.gdp || '#4ecdc4')
        .on('mouseover', function(event, d) {
            tooltip.transition()
                .duration(200)
                .style('opacity', .9);
            tooltip.html(`Year: ${d.year}<br/>GDP: ${d.gdp.toLocaleString()} M`)
                .style('left', (event.pageX + 5) + 'px')
                .style('top', (event.pageY - 28) + 'px');
        })
        .on('mouseout', function() {
